...
moveServosTo = function (x, y, z) {
  ...
  servo1.to(
    angles[1].map(
      config.servo1.in_min,
      config.servo1.in_max,
      config.servo1.out_min,
      config.servo1.out_max
    )
  );
  servo2.to(
    angles[2].map(
      config.servo2.in_min,
      config.servo2.in_max,
      config.servo2.out_min,
      config.servo2.out_max
    )
  );
  servo3.to(
    angles[3].map(
      config.servo3.in_min,
      config.servo3.in_max,
      config.servo3.out_min,
      config.servo3.out_max
    )
  );
  console.log(angles);
};
...
go = function (x, y, z, easeType) {
  ...
  } else if (!easeType) easeType = defaultEaseType; //If no easeType is specified, go with default (specified in config.js)
